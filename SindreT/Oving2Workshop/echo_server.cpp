    ...
    async_read_until(connection->socket, *read_buffer, "\r\n",
      ...
      if (!ec) {
        ...
        if(message == "GET / HTTP/1.1"){
          string html = "<title>Hovedside</title><p>Dette er hovedsiden</p><a href='/en_side'><p>Link til en side</p></a>";
          write_stream << "HTTP/1.1 200 OK\r\nContent-Length:" << html.length() << "\r\n\r\n" << html << "\r\n";
        }else if(message == "GET /en_side HTTP/1.1"){
          string html = "<title>En side</title><p>Dette er en side</p><a href='/'><p>Link til startsiden</p></a>";
          write_stream << "HTTP/1.1 200 OK\r\nContent-Length:" << html.length() << "\r\n\r\n" << html << "\r\n";
        }else{
          string html = "<title>404 Error</title><p>404 Not Found</p>";
