...
beginTil:
	...
	if fraKonto.Saldo >= belop {
		...
		//db.Save(&fraKonto)
		//db.Save(&tilKonto)
		newvFra := fraKonto.Version + 1
		newvTil := tilKonto.Version + 1
		newSaldoFra := fraKonto.Saldo
		newSaldoTil := tilKonto.Saldo

		tx := db.Begin()
		if rows := tx.Model(&fraKonto).Where("version = ?", fraKonto.Version).Updates(&Konto{Saldo: newSaldoFra, Version: newvFra}).RowsAffected; rows == 0 {
			tx.Rollback()
			fmt.Print("Noe gikk galt. Prøv igjen senere")
			return
		}

		if rows := tx.Model(&tilKonto).Where("version = ?", tilKonto.Version).Updates(&Konto{Saldo: newSaldoTil, Version: newvTil}).RowsAffected; rows == 0 {
			tx.Rollback()
			fmt.Print("Noe gikk galt. Prøv igjen senere")
			return
		}
